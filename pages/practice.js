...
export default function Practice({ books }) {
  ...
  }, [selectedTopic, difficulty, perPage])
  ...
  async function loadQuestions(page = 1) {
    ...
    setTotalPages(perPage > 0 ? Math.ceil((count || 0) / perPage) : 1)
    setLoading(false)
  }
  ...
  return (
    <div className="quiz-card rounded-2xl p-6 md:p-8 shadow-xl mb-8">
      ...
      <div className="flex justify-end items-center mb-4">
        <label className="text-sm font-semibold text-gray-700 mr-2">Show:</label>
        <select 
          value={perPage}
          onChange={(e) => setPerPage(parseInt(e.target.value))}
          className="p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm"
        >
      ...
      <div className="space-y-4 mt-2">
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-500">
            {selectedTopic ? 'No questions found for the selected filters' : 'Select a book and topic to view questions'}
          </div>
        ) : (
          questions.map((q, idx) => {
            const globalNumber = perPage > 0 ? (currentPage - 1) * perPage + idx + 1 : idx + 1
            ...
            return (
              <div key={q.id} className="bg-white rounded-xl p-5 shadow-sm border border-gray-100 practice-card">
                ...
                <p className="font-medium text-gray-800 mb-4 question-text">{q.text}</p>
                ...
                {q.explanation && (
                  <div className="mt-3 p-3 bg-blue-50 rounded-lg text-sm text-gray-600 explanation-text">
      ...
      {totalQuestions > 0 && (
        <p className="text-sm text-gray-500 mt-4">Total: {totalQuestions} questions</p>
      )}
